...
export default class GameBoy extends EventEmitter {
  ...
  iterationEndRoutine() {
    if ((this.stopEmulator & 0x1) === 0) {
      // Re-apply persistent cheats once per emulated frame (GameShark style).
      this.cheatManager.applyCheats();
  ...
  */
  ...
  // Public memory/cheat API for external tools (HTML UI, debuggers, etc.).
  readByte(address: string | number) {
    return this.cheatManager.readByte(address);
  }

  writeByte(address: string | number, value: string | number) {
    this.cheatManager.writeByte(address, value);
  }

  addCheat(name: string, address: string | number, value: string | number) {
    return this.cheatManager.addCheat(name, address, value);
  }

  removeCheat(id: number) {
    return this.cheatManager.removeCheat(id);
  }

  enableCheat(id: number) {
    return this.cheatManager.enableCheat(id);
  }

  disableCheat(id: number) {
    return this.cheatManager.disableCheat(id);
  }
  ...
  setMemoryPatches(
    ...
    this.cheatManager.setMemoryPatches(patches);
  }

  clearMemoryPatches() {
    this.cheatManager.clearMemoryPatches();
  }

  getCheats() {
    return this.cheatManager.getCheats();
  }
