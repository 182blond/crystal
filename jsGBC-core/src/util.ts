...
export async function readFirstMatchingExtension(
  ...
  // Always unpack .zip and pick the first ROM inside matching the extensions.
  if (hasExtension(filename, "zip")) {
    const decodedZip = await JSZip.loadAsync(buffer);
    const filenames = Object.keys(decodedZip.files);
    const validFilenames = filenames.filter(x =>
      extensions.find(extension => hasExtension(x, extension))
    );
...
export async function readCartridgeROM(
  blob: Blob,
  filename: string = ""
): Promise<ArrayBuffer> {
  return readFirstMatchingExtension(blob, filename, ["gbc", "gb"]);
}
