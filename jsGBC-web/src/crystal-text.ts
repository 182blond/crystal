...
export const CRYSTAL_NICKNAME_LENGTH = 11;
const TERMINATOR = 0x50;

/** ROM-style species names that differ from gameshark.txt labels. */
const SPECIES_ROM_NAME_OVERRIDES: Record<string, string> = {
  "1D": "NIDORAN♀",
  "20": "NIDORAN♂",
  "53": "FARFETCH'D",
  FA: "HO-OH"
...
export function encodeCrystalDefaultName(text: string): number[] {
  const bytes: number[] = [];
...
export function speciesRomName(speciesId: string, catalogName: string): string {
  const override = SPECIES_ROM_NAME_OVERRIDES[speciesId.toUpperCase()];
  if (override) {
    return override;
  }
...
export function encodeDefaultSpeciesNickname(
  speciesId: string,
  catalogName: string
): number[] {
...
export function nicknameBytesEqual(a: number[], b: number[]): boolean {
